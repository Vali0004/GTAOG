...
buf_t sstrstr(ccp haystack, ccp needle) {
	if (*needle == '\0') {
		return (buf_t)haystack;
	}
	while (*haystack) {
		ccp haystackPtr = haystack;
		ccp needlePtr = needle;
		while (*needlePtr && *haystackPtr == *needlePtr) {
			haystackPtr++;
			needlePtr++;
		}
		if (*needlePtr == '\0') {
...
string& string::operator=(const char* other) {
	size_ = strlen(other) + 1;
	buf_t tmpAlloc = new char[size_];
	strncpy(tmpAlloc, other, size_);
	data_ = tmpAlloc;
	return *this;
}